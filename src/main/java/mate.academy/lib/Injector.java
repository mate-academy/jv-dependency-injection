...
public class Injector {
    ...
    public Object getInstance(Class<?> interfaceClazz) {
        ...
        if (!clazz.isAnnotationPresent(Component.class)) {
            throw new RuntimeException("Injection failed, missing @Component "
                    + "annotation on the class " + interfaceClazz.getName());
        }
        for (Field field: declaredFields) {
            if (field.isAnnotationPresent(Inject.class)) {
                Object fieldInstance = getInstance(field.getType());
                clazzImplementation = createClazzImplementation(clazz);
                try {
                    field.setAccessible(true);
                    field.set(clazzImplementation, fieldInstance);
                } catch (IllegalAccessException e) {
                    throw new RuntimeException("Can't initialize field: "
                            + field.getName() + ", in class: " + clazz.getName(),e);
                }
            }
        }
    ...
    private Object createClazzImplementation(Class<?> clazz) {
