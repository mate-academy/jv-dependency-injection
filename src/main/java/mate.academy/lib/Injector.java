...
public class Injector {
    ...
    private Class<?> findImplementation(Class<?> interfaceClazz) throws RuntimeException {
        ...
        if (interfaceClazz.isInterface()) {
            wrongClassCheck(interfaceImplementations.get(interfaceClazz));
            return interfaceImplementations.get(interfaceClazz);
        }
        wrongClassCheck(interfaceClazz);
        return interfaceClazz;
    }

    private void wrongClassCheck(Class<?> checkedClass) {
        if (!checkedClass.isAnnotationPresent(Component.class)) {
