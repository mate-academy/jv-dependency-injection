...
public class Injector {
    ...
    public Object getInstance(Class<?> interfaceClazz) {
        ...
        Object clazzmplementationInstance = null;
        Field[] declaredFields = clazz.getDeclaredFields();
        for (Field field : declaredFields) {
            if (field.isAnnotationPresent(Inject.class)) {
                Object fieldInstance = getInstance(field.getType());
                clazzmplementationInstance = createNewInstance(clazz);
                try {
                    field.setAccessible(true);
                    field.set(clazzmplementationInstance, fieldInstance);
        ...
        if (clazzmplementationInstance == null) {
            clazzmplementationInstance = createNewInstance(clazz);
        }
        return clazzmplementationInstance;
    }
