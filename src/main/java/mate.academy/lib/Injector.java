...
public class Injector {
    ...
    public Object getInstance(Class<?> interfaceClazz) {
        Object clazzImplementationInterface = null;
        ...
        for (Field field : declaredFields) {
            if (field.isAnnotationPresent(Inject.class)) {
                ...
                clazzImplementationInterface = createNewInstance(clazz);
                try {
                    field.setAccessible(true);
                    field.set(clazzImplementationInterface, fieldInstance);
                } catch (IllegalAccessException e) {
                    throw new RuntimeException("Can`t set field -> " + field.getName(), e);
                }
            }
        }
        if (clazzImplementationInterface == null) {
            clazzImplementationInterface = createNewInstance(clazz);
        }
        return clazzImplementationInterface;
    }
